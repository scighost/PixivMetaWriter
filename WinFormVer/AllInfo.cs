...
class AllInfo
{
    ...
    public PixivInfo PixivInfo { get; set; }
    public BitmapMetadata metadata { get; set; }
    public string Pid_Page { get; set; }
    public AllInfo(FileInfo fileInfo)
    {
        this.FileInfo = fileInfo;
        Match match = Regex.Match(fileInfo.Name, "[0-9]+_p[0-9]");
        this.Pid_Page = match.Value;
    }
}
class PixivInfo
{
    ...
    public BitmapMetadata metadata = new BitmapMetadata("jpg");//元数据
    ...
    {
        BitmapMetadata _metadata = new BitmapMetadata("jpg");
        _metadata.Author = new ReadOnlyCollection<string>(new List<string>() { User });
        _metadata.Title = Title;
        _metadata.Comment = Intro;
        _metadata.DateTaken = Date;
        _metadata.Keywords = Tags.AsReadOnly();
        //todo _metadata.Rating = SetRating();
        return _metadata;
    }
